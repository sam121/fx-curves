...
MODES=("BALANCE" "BANK_TRANSFER")  # payIn fixed to BALANCE for both
MAX_USD_NOTIONAL="${MAX_USD_NOTIONAL:-10000000}"   # cap by USD notional
...
if [[ -z "$PROFILE_ID" || "$PROFILE_ID" == "null" ]]; then
  echo "Error: could not resolve Wise profile ID via /v1/profiles"
  exit 1
fi
...
# Prefetch SRC->USD mid for ladder conversion & cap
...
# Build ladders per source from USD anchors (rounded DOWN)  <-- FIXED: append to AMTS
...
for SRC in "${CCYS[@]}"; do
  ...
  if [[ -n "$rate_usd" && "$rate_usd" != "0" ]]; then
    for usd in "${USD_ANCHORS[@]}"; do
      ...
      [[ -z "$amt" || "$amt" -lt 1 ]] && amt=1
      AMTS+=("$amt")   # <-- this line was missing before; without it ladders were empty
    done
  else
...
# --- snapshot the run so history persists in the repo ---
SNAPDIR="data/snapshots"
mkdir -p "$SNAPDIR"
SNAPFILE="${SNAPDIR}/$(date -u +%Y-%m-%dT%H%M%SZ).json"
cp "$OUTFILE" "$SNAPFILE"

# Maintain/append a simple manifest of available snapshots
MANIFEST="${SNAPDIR}/manifest.json"
if [[ -s "$MANIFEST" ]]; then
  jq --arg f "$(basename "$SNAPFILE")" \
     --arg ts "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
     '. + [{file:$f, ts:$ts}]' "$MANIFEST" > "${MANIFEST}.tmp" && mv "${MANIFEST}.tmp" "$MANIFEST"
else
  jq -n --arg f "$(basename "$SNAPFILE")" \
        --arg ts "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
        '[{file:$f, ts:$ts}]' > "$MANIFEST"
fi
